...
class DataFrameThread(QThread):
    ...
    def __init__(self,total_data,CHUNK=128):
        ...
        self.total_data=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]]        
        self.Acq = True
        self.fs = 2000000.0   # sample rate = 2Msps
        #self.ts = 1.0/self.fs # sampling interval
        #self.samples = 2048   # samples
    @pyqtSlot()
    def stop(self):
    ...
    def get_rawdata(self):
        ...
        for i in range(self.CHUNK):
            for j in range(16): #16 channels
                tmp = frames[i].ADCdata[j]&0xffff
#                tmp = tmp - (int(tmp<<1)&(2**16)) #convert from '2s" number
                chns[j].append(tmp)  
...
#                chns[j].append(tmp)   
        ...
        for i in range(len(self.total_data)):
            self.total_data[i] = np.concatenate([self.total_data[i], chns[i]]) #12-bit mode
            if len(self.total_data[i]) > self.MAX_PLOT_SIZE:
                self.total_data[i] = self.total_data[i][self.CHUNK:] 
        return chns, self.total_data 
    
    def chns_rfft(self,total_data):
        pn=[]
        fn=[]
    ...
    def stream(self):
        while self.running():
            self.mutex.lock()
            chns, total_data = self.get_rawdata()
            fn,pn = self.chns_rfft(total_data)
            self.result = [chns,fn,pn]
...
class DacScanThread(QThread):
    ...
    def __init__(self,reg,averageN,step):
        ...
        self.result = [[],[]]      
    def run(self):
        for code in range(0,256,self.step): #fixed a bug here
            self.mutex.lock()
...
class XtalkScanThread(QThread):
    ...
    def peak_to_peak(self, chns_truncate):
        ...
        for i in range(len(self.hist_chns)):    
            mu = np.mean(self.hist_chns[i]) #calculate peak-to-peak
            sigma = np.std(self.hist_chns[i])
            self.hist_result[i] = np.histogram(self.hist_chns[i]),mu,sigma       
...
class NoiseScanThread(QThread):
    ...
    def histogram(self, chn_data):
        for i in range(len(self.hist_chn)):
            self.hist_chn[i] = self.hist_chn[i] + chn_data[i]
            mu = np.mean(self.hist_chn[i])
            sigma = np.std(self.hist_chn[i])
            self.hist_result[i] = np.histogram(self.hist_chn[i]),mu,sigma
#            self.hist_result[i].append(np.mean(self.hist_chn[i]))
#            self.hist_result[i].append(np.std(self.hist_chn[i]))
        return self.hist_result
    
    def stream(self):
        while self.running():
            self.mutex.lock()
            tem = self.get_rawdata()
            self.result = self.histogram(tem)
            self.mutex.unlock()
            self.interact.emit(self.result)
            QThread.msleep(2)
        self.sgnFinished.emit()
...
#        layout5.addWidget(self.btn_CodeScanStop)            
    ...
    def ViewControl(self):
        ...
        #----------------------------------------#
        lbl_xtalk_statistics = QLabel("Xtalk/run")
        self.btn_XtalkRunStart = QPushButton("Run")
        self.btn_XtalkRunStart.clicked.connect(self.btn_XtalkRunStart_clicked)
        self.btn_XtalkRunStop  = QPushButton("Stop")
        self.btn_XtalkRunStop.clicked.connect(self.btn_XtalkRunStop_clicked)
        #----------------------------------------#
        ...
        layout.addWidget(lbl_xtalk_statistics,2,0)
        layout.addWidget(self.btn_XtalkRunStart,2,1)
        layout.addWidget(self.btn_XtalkRunStop,2,2)
        
        layout.addWidget(lbl_noise_statistics,3,0)
        layout.addWidget(self.btn_NoiseRunStart,3,1)
        layout.addWidget(self.btn_NoiseRunStop,3,2)
    ...
    def COTS_ADC(self):
        ...
        #Add Vmonitor and imonitor
        lbl_v_mon = QLabel("vmonitor")
        self.cbox_v_mon = QComboBox()
        self.cbox_v_mon.addItem("VBGR")#000
        self.cbox_v_mon.addItem("VCMI")#001
        self.cbox_v_mon.addItem("VCMO")#010
        self.cbox_v_mon.addItem("VREFP")#011
        self.cbox_v_mon.addItem("VREFN")#100
        self.cbox_v_mon.addItem("VSSA") #110,111
        self.cbox_v_mon.activated[str].connect(self.fun_v_mon)  #   
        
        #lbl_i_mon = QLabel("imonitor")
        self.cbox_i_mon = QComboBox()
        self.cbox_i_mon.addItem("ICMOS_REF_5k") #000
        self.cbox_i_mon.addItem("ISHA0_5k") #001
        self.cbox_i_mon.addItem("IADC0_5k") #010
        self.cbox_i_mon.addItem("ISHA1_5k") #011
        self.cbox_i_mon.addItem("IADC1_5k") #100
        self.cbox_i_mon.addItem("IBUFF_CMOS")#101
        self.cbox_i_mon.addItem("IREF_5k")   #110
        self.cbox_i_mon.addItem("IREFBUFFER0")#111
        self.cbox_i_mon.activated[str].connect(self.fun_i_mon)  #   
        
        #checkable btn for turnning on/off vmonitor and imontior
        self.btn_v_mon_switch = QPushButton("vmon")
        self.btn_v_mon_switch.setCheckable(True)
        self.btn_v_mon_switch.toggle()
        self.btn_v_mon_switch.setChecked(False)
        self.btn_v_mon_switch.clicked.connect(self.btn_v_mon_switch_clicked)        

        #checkable btn for turnning on/off vmonitor and imontior
        self.btn_i_mon_switch = QPushButton("imon")
        self.btn_i_mon_switch.setCheckable(True)    
        self.btn_i_mon_switch.toggle()
        self.btn_i_mon_switch.setChecked(False)
        self.btn_i_mon_switch.clicked.connect(self.btn_i_mon_switch_clicked)
        #---------------------------------------------#
        ...
        layout.addWidget(lbl_v_mon,1,0)
        layout.addWidget(self.cbox_v_mon,1,1)
        layout.addWidget(self.btn_v_mon_switch,1,2)
        #layout.addWidget(lbl_i_mon,1,3)
        layout.addWidget(self.cbox_i_mon,1,3)
        layout.addWidget(self.btn_i_mon_switch,1,4)
        
        layout.addWidget(lbl_average,2,0)
        layout.addWidget(self.ledit_cots_avr_points,2,1)
        
        layout.addWidget(lbl_value,2,2)
        layout.addWidget(self.ledit_cotsadc_result,2,3)
        layout.addWidget(self.btn_read_cots_adc,2,4)
        
        layout.addWidget(lbl_steps,3,0)
        layout.addWidget(self.sp_dac_steps,3,1)
        layout.addWidget(self.btn_dacscan_start,3,2)
        layout.addWidget(self.btn_dacscan_stop,3,3)
    ...
    def ControlTabWidget(self):
        ...
        self.ledit_period = QLineEdit("200")  
        ...
        self.ledit_VREFP = QLineEdit("0xe4")
        self.ledit_VREFN = QLineEdit("0x24")
        self.ledit_VCMI  = QLineEdit("0x60") #change here to 0x60, original 0x5c
        self.ledit_VCMO  = QLineEdit("0x82")
        ...
        self.cbox_ioffset_vrefp.setCurrentIndex(1)
        ...
        self.cbox_ioffset_vrefn.setCurrentIndex(1)
        ...
        self.cbox_ioffset_vcmi.setCurrentIndex(1)
        ...
        self.cbox_ioffset_vcmo.setCurrentIndex(1)
        ...
        self.btn_set_bjt_warm_default = QPushButton("Warm values")
        self.btn_set_bjt_warm_default.clicked.connect(self.btn_set_bjt_warm_default_clicked)
        
        self.btn_set_bjt_cold_default = QPushButton("Cold values")
        self.btn_set_bjt_cold_default.clicked.connect(self.btn_set_bjt_cold_default_clicked)
        ...
        BJT_layout.addWidget(self.btn_set_bjt_warm_default,4,0)
        BJT_layout.addWidget(self.btn_SetRefVol,4,1)
        BJT_layout.addWidget(self.btn_set_bjt_cold_default,4,2)
...
#
        ...
        self.ledit_CMOS_ibuff0  = QLineEdit("0x27")
        self.ledit_CMOS_ibuff1  = QLineEdit("0x27")

        self.btn_SetCMOSRefV = QPushButton("Set Ref")
        self.btn_SetCMOSRefV.clicked.connect(self.btn_SetCMOSRefV_clicked) 

        self.btn_set_cmos_warm_default = QPushButton("Warm value")
        self.btn_set_cmos_warm_default.clicked.connect(self.btn_set_cmos_warm_default_clicked) 
        
        self.btn_set_cmos_cold_default = QPushButton("Cold value")
        self.btn_set_cmos_cold_default.clicked.connect(self.btn_set_cmos_cold_default_clicked)         
        ...
        CMOS_layout.addWidget(self.btn_set_cmos_warm_default,5,1)
        CMOS_layout.addWidget(self.btn_set_cmos_cold_default,5,2)
        ...
        self.ledit_samples_avr = QLineEdit("2000")
        
        lbl_run_times = QLabel("Run times")
        self.ledit_run_time = QLineEdit("1")
        
        lbl_cali_data = QLabel("savecali")
        self.cbox_save_cali = QComboBox()
        self.cbox_save_cali.addItem("None")
        self.cbox_save_cali.addItem("savefig")
        #self.cbox_save_cali.activated[str].connect(self.fun_save_califig)        
        ...
        Cali_layout.addWidget(lbl_cali_data,1,2)
        Cali_layout.addWidget(self.cbox_save_cali,1,3)
        Cali_layout.addWidget(self.lbl_calistatus,1,4)
...
#        self.time_curvef = self.data_plot.plot(pen='y',symbolBrush=(119,172,48),symbolPen='w',symbol='+',symbolSize=4,  name = "Chf")
        ...
        tab2 = QWidget()
        tab2_win = pg.GraphicsLayoutWidget()
        ...
        #----------tab3 DAC SCAN-----------#
        tab3 = QWidget()
        tab3_win = pg.GraphicsLayoutWidget()
        ...
        self.tab3_p1.showGrid(x=True,y=True)
        ...
        #-------------------tab5 Xtalk------------------# 
        tab5 = QWidget()
        tab5_win = pg.GraphicsLayoutWidget()
        
        self.noise_chn=[]      
        for i in range(16):
            self.noise_chn.append(tab5_win.addPlot(title="Chn"+str(i)))
            if(i==3 or i==7 or i == 11):
                tab5_win.nextRow()
        
#        self.chn_noise_text=[]
#        for i in range(16):
#            self.chn_noise_text.append(pg.TextItem("mu,sigma"))
        self.noise=[]
        for i in range(16):
            self.noise_chn[i].showGrid(x=True,y=True)
#            chn[i].addItem(self.chn_noise_text[i])
#            self.chn_noise_text[i].setPos(0,0)
            self.noise.append(self.noise_chn[i].plot(pen=(19,234,201))) 
        
        tab5_layout = QGridLayout()
        tab5_layout.addWidget(tab5_win)
        tab5.setLayout(tab5_layout)
        
        #-----------------tab6 gain calibration---------------#
        tab6 = QWidget()
        tab6_win = pg.GraphicsLayoutWidget()
        
        tab6_layout = QGridLayout()
        tab6_layout.addWidget(tab6_win)
        tab6.setLayout(tab6_layout)
        
        self.VisualTab.addTab(tab1,"&Wave/FFT")
        self.VisualTab.addTab(tab2,"DNL/INL")
        self.VisualTab.addTab(tab3,"ref DAC/SCAN")
        self.VisualTab.addTab(tab4,"Xtalk")
        self.VisualTab.addTab(tab5,"Noise")
        self.VisualTab.addTab(tab6,"Gain Cali")
    #--------------------Action----------------------#    
    #----------------COTS ADC related----------------#
    ...
    def fun_v_mon(self,text):
        self.brd_config.cost_adc_v_mon_select(text)
    def fun_i_mon(self,text):
        self.brd_config.cost_adc_i_mon_select(text)
    
    def btn_v_mon_switch_clicked(self):
        if self.btn_v_mon_switch.isChecked():
            self.brd_config.cost_adc_v_mon_ena(1)
        else:
            self.brd_config.cost_adc_v_mon_ena(0)
            
    def btn_i_mon_switch_clicked(self):
        if self.btn_i_mon_switch.isChecked():
            self.brd_config.cost_adc_i_mon_ena(1)
        else:
            self.brd_config.cost_adc_i_mon_ena(0)
...
##--------code for thread-----------#          
    ...
    @pyqtSlot()
    def btn_set_cmos_warm_default_clicked(self):
        self.ledit_CMOS_VREFP.setText("0xce")
        self.ledit_CMOS_VREFN.setText("0x2b")
        self.ledit_CMOS_VCMI.setText("0x5b")
        self.ledit_CMOS_VCMO.setText("0x7b")
        
    @pyqtSlot()
    def btn_set_cmos_cold_default_clicked(self):
        self.ledit_CMOS_VREFP.setText("0xc6")
        self.ledit_CMOS_VREFN.setText("0x30")
        self.ledit_CMOS_VCMI.setText("0x5b")
        self.ledit_CMOS_VCMO.setText("0x7b")    
    ...
    def btn_autocali_clicked(self):
        ...
        #add a flag that storage figs or rawdata
        flag = self.cbox_save_cali.currentText()
        self.lbl_calistatus.setText("Process")        
        for i in range(runs):
            self.brd_config.adc_autocali(samples,flag)
    ...
    @pyqtSlot()
    def btn_set_bjt_warm_default_clicked(self):
        self.ledit_VREFP.setText("0xe4")
        self.ledit_VREFN.setText("0x24")
        self.ledit_VCMI.setText("0x5c")
        self.ledit_VCMO.setText("0x82")
    @pyqtSlot()
    def btn_set_bjt_cold_default_clicked(self):
        self.ledit_VREFP.setText("0xf1")
        self.ledit_VREFN.setText("0x29")
        self.ledit_VCMI.setText("0x65")
        self.ledit_VCMO.setText("0x8d")        
    ...
    @pyqtSlot()
    def btn_XtalkRunStart_clicked(self):
        self.brd_config.Acq_start_stop(1) #Acq start   
        self.Xtalkthread = QThread()       
        self.XtalkFrame = XtalkScanThread(self.CHUNK*16) #4096 points PKT num is limited to 174 pkts, otherwise get wrong, get channel 0 by default
        self.XtalkFrame.moveToThread(self.Xtalkthread)
        self.XtalkFrame.interact.connect(self.Update_Xtalk_scan)
        self.Xtalkthread.started.connect(self.XtalkFrame.stream)
        self.Xtalkthread.start() 
        self.btn_XtalkRunStart.setDisabled(True)
        self.btn_XtalkRunStop.setEnabled(True) 
         
    @pyqtSlot()    
    def btn_XtalkRunStop_clicked(self):
        self.XtalkFrame.stop()
        self.Xtalkthread.quit()
        self.Xtalkthread.wait()
        self.brd_config.Acq_start_stop(0) #Acq stop 
        time.sleep(0.2)
        self.brd_config.Acq_start_stop(0) #Acq stop        
        self.btn_XtalkRunStart.setEnabled(True)
        self.btn_XtalkRunStop.setDisabled(True)           
    ...
    def btn_NoiseRunStart_clicked(self):
        ...
        self.NoiseFrame = NoiseScanThread(self.CHUNK) #4096 points PKT num is limited to 174 pkts, otherwise get wrong, get channel 0 by default
...
#            self.time_curvef.setData(chns_data[0xf])
    ...
    def Update_histo(self,result1):
        if self.CodeFrame.mutex.tryLock():
            ...
            self.p1.setXRange(xcor[1][0],xcor[1][1]) 
            
            self.ADC0_hist_0.setData(xdata[1])
...
#            self.ADC0_hist_7.setData(xdata[7])
            ...
            self.p2.setXRange(xcor[9][0],xcor[9][1]) 
            self.ADC1_hist_0.setData(xdata[9])
...
#            
    def Update_Xtalk_scan(self,result1): 
        if self.XtalkFrame.mutex.tryLock():
            xdata = copy(result1)
            self.XtalkFrame.mutex.unlock()                      
            self.ADC_xtalk_0.setData(xdata[0][0][1],xdata[0][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_1.setData(xdata[1][0][1],xdata[1][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_2.setData(xdata[2][0][1],xdata[2][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_3.setData(xdata[3][0][1],xdata[3][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_4.setData(xdata[4][0][1],xdata[4][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_5.setData(xdata[5][0][1],xdata[5][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_6.setData(xdata[6][0][1],xdata[6][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_7.setData(xdata[7][0][1],xdata[7][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_8.setData(xdata[8][0][1],xdata[8][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_9.setData(xdata[9][0][1],xdata[9][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_10.setData(xdata[10][0][1],xdata[10][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_11.setData(xdata[11][0][1],xdata[11][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_12.setData(xdata[12][0][1],xdata[12][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_13.setData(xdata[13][0][1],xdata[13][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_14.setData(xdata[14][0][1],xdata[14][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            self.ADC_xtalk_15.setData(xdata[15][0][1],xdata[15][0][0], stepMode=True, fillLevel=0, brush=(122,0,120,150))
            
            self.xtalk_chn0.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[0][1],xdata[0][2]))
            self.xtalk_chn1.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[1][1],xdata[1][2]))
            self.xtalk_chn2.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[2][1],xdata[2][2]))
            self.xtalk_chn3.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[3][1],xdata[3][2]))
            self.xtalk_chn4.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[4][1],xdata[4][2]))
            self.xtalk_chn5.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[5][1],xdata[5][2]))
            self.xtalk_chn6.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[6][1],xdata[6][2]))
            self.xtalk_chn7.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[7][1],xdata[7][2]))
            self.xtalk_chn8.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[8][1],xdata[8][2]))
            self.xtalk_chn9.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[9][1],xdata[9][2]))
            self.xtalk_chn10.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[10][1],xdata[10][2]))
            self.xtalk_chn11.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[11][1],xdata[11][2]))
            self.xtalk_chn12.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[12][1],xdata[12][2]))
            self.xtalk_chn13.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[13][1],xdata[13][2]))
            self.xtalk_chn14.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[14][1],xdata[14][2]))
            self.xtalk_chn15.setTitle('\u03BC=%.1f,\u03c3=%.2f'%(xdata[15][1],xdata[15][2]))
    ...
    def Update_Noise_scan(self,result1):
        ...
        if self.NoiseFrame.mutex.tryLock():
            ...
            self.noise[0].setData(result[0][0][1],result[0][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[1].setData(result[1][0][1],result[1][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[2].setData(result[2][0][1],result[2][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[3].setData(result[3][0][1],result[3][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[4].setData(result[4][0][1],result[4][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[5].setData(result[5][0][1],result[5][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[6].setData(result[6][0][1],result[6][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[7].setData(result[7][0][1],result[7][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[8].setData(result[8][0][1],result[8][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[9].setData(result[9][0][1],result[9][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[10].setData(result[10][0][1],result[10][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[11].setData(result[11][0][1],result[11][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[12].setData(result[12][0][1],result[12][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[13].setData(result[13][0][1],result[13][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[14].setData(result[14][0][1],result[14][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            self.noise[15].setData(result[15][0][1],result[15][0][0],stepMode=True, fillLevel=0, brush=(0,0,255,150))
            for i in range(16):
                self.noise_chn[i].setTitle('\u03BC=%.1f,\u03c3=%.2f'%(result[i][1],result[i][2]))
